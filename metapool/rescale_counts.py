...
def convert_read_count_to_cell_count_per_g_input(df,
                                                 metadata_features,
                                                 prep_info_samples):
    lengths = metadata_features.loc[df.index, "total_length"]
    sample_weights = prep_info_samples.loc[df.index,
    ...
    mult_row = (6.022 * (10.0 ** 23)) / \
        (lengths * (650 * 10.0 ** 9) * sample_weights)
