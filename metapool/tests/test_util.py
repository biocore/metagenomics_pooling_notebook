...
class SeqproTests(unittest.TestCase):
    def setUp(self):
        ...
        self.vf_test_dir = os.path.join(tests_dir, "metapool",
                                        "tests", "VFTEST")
    ...
    def test_fastp_run(self):
        ...
        obs = generate_prep_from_sample_sheet(self.fastp_run,
                                              self.fastp_sheet,
                                              "./")

        # confirm that the filenames are generated properly and are as
        # expected.
        for _, prep_path in obs:
            obs_prep_file_name = split(prep_path)[1]
            self.assertIn(obs_prep_file_name, exp_preps)

        # assert the contents of each file are correct,
    ...
    def test_legacy_run(self):
        ...
        for _, prep_path in obs:
            obs_prep_file_name = split(prep_path)[1]
            self.assertIn(obs_prep_file_name, prep_path)
        ...
        exp = {
            ...
            "200318_A00953_0082_AH5TWYDSXY.Trojecp_666.3.tsv": {
                0: {
                    ...
                    "center_project_name": "Trojecp",
                    "experiment_design_description": "SomethingWitty",
                    "fraction_passing_quality_filter": 5.333333333333333e-05,
                    "i5_index_id": "iTru5_01_A",
                    "i7_index_id": "iTru7_107_11",
                    "index": "CAATGTGG",
                    "index2": "GGTACGAA",
                ...
                1: {
                    ...
                    "center_project_name": "Trojecp",
                    "experiment_design_description": "SomethingWitty",
                    "fraction_passing_quality_filter": 4e-05,
                    "i5_index_id": "iTru5_01_A",
                    "i7_index_id": "iTru7_107_12",
                    "index": "AAGGCTGA",
                    "index2": "CGATCGAT",
                ...
                2: {
                    ...
                    "center_project_name": "Trojecp",
                    "experiment_design_description": "SomethingWitty",
                    "fraction_passing_quality_filter": 2.8218694885361552e-05,
                    "i5_index_id": "iTru5_01_A",
                    "i7_index_id": "iTru7_107_13",
                    "index": "TTACCGAG",
                    "index2": "AAGACACC",
...
class SeqproBCLConvertTests(unittest.TestCase):
    def setUp(self):
        ...
        tests_dir = os.path.abspath(os.path.dirname(__file__))
        tests_dir = os.path.dirname(os.path.dirname(tests_dir))
        self.data_dir = os.path.join(tests_dir, "metapool", "tests", "data")
        self.fastp_run = os.path.join(
    ...
    def test_fastp_run(self):
        results = generate_prep_from_sample_sheet(self.temp_copy,
                                                  self.fastp_sheet,
                                                  "./")

        exp_lengths = {'200418_A00953_0082_AH5TWYDSXY.Project_1111.1.tsv': 4,
                       '200418_A00953_0082_AH5TWYDSXY.Project_1111.3.tsv': 4,
                       '200418_A00953_0082_AH5TWYDSXY.Trojecp_666.3.tsv': 5}

        obs_lengths = {}
        for _, prep_file_path in results:
            with open(prep_file_path) as f:
                file_name = split(prep_file_path)[1]
                # use file_name over qiita_id since two preps belong to the
                # same project.
                obs_lengths[file_name] = len(f.read().split("\n"))

        self.assertDictEqual(obs_lengths, exp_lengths)
...
class MetricsTest(unittest.TestCase):
    ...
    def test_metrics_generation(self):
        ...
        results = generate_prep_from_sample_sheet(self.run,
                                                  self.sheet,
                                                  out_dir)

        self.assertTrue(len(results), 1)

        obs_prep_path = results[0][1]

        # confirm output prep-info file exists and is named as expected.
        exp = join(out_dir, "240124_LH00444_0046_A223N22LT4.NPH_15288.7.tsv")

        self.assertEqual(exp, obs_prep_path)
        self.assertTrue(exists(obs_prep_path))

        # read in observed result and determine whether it matches
        # expectations.
        obs = pd.read_csv(obs_prep_path, sep="\t", dtype=str)
