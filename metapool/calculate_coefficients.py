...
def calculate_coefficients(table_synthetic_hits, metadata_pools, dilutions, plot_fit=False):
    # Strip leading X in sample names which are automatically removed by R table reading.
    ...
    table_synthetic_hit_totals = pd.DataFrame(data={"sample_name_r": table_synthetic_hit_totals.index.to_list(), "table_synthetic_hit_totals": table_synthetic_hit_totals.values.tolist()}, index=None)

    # Convert feature-table to long-form and merge with total number of hits to plasmid sequences
    table_synthetic_hits_long_with_totals_beta = pd.melt(table_synthetic_hits.reset_index(), id_vars="OTUID")
    table_synthetic_hits_long_with_totals_beta.columns = ["plasmid_id", "sample_name_r", "count"]
    table_synthetic_hits_long_with_totals = table_synthetic_hits_long_with_totals_beta.merge(table_synthetic_hit_totals, on="sample_name_r")

    # Merge updated feature-table with sample-pool information
    table_synthetic_hits_long_with_totals_and_pools = table_synthetic_hits_long_with_totals.merge(metadata_pools, on="sample_name_r")
    ...
    table_synthetic_hits_long_with_totals_and_pools_all_lanes["counts_per_million_log10"] = np.log10(table_synthetic_hits_long_with_totals_and_pools_all_lanes["counts_per_million"])

    # Merge table with plasmid dilution information
    table_synthetic_hits_long_with_totals_and_pools_all_lanes_with_dilutions = table_synthetic_hits_long_with_totals_and_pools_all_lanes.merge(dilutions, on=["plasmid_id", "pool"])
    ...
    for name, group in final_table_1.groupby(["pool", "sample_name_pool"]):
        m, b, r, p, std_err = scipy.stats.linregress(group["counts_per_million_log10"], group["dilution_id"])
